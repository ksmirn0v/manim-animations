...
class AnimationManager:
    ...
    def _set_constants(self, config: dict):
        ...
        self._ANIMATION_SPEED_BAR_MIN_SELECT = config.get("ANIMATION_SPEED_BAR_MIN_SELECT", self._ANIMATION_SPEED)
        ...
        self._BAR_SELECT_MIN_FILL_COLOR = config.get("BAR_SELECT_MIN_FILL_COLOR", self._BAR_SELECT_FILL_COLOR)
        self._BAR_SELECT_MIN_FILL_OPACITY = config.get("BAR_SELECT_MIN_FILL_OPACITY", self._BAR_SELECT_FILL_OPACITY)
        self._BAR_SELECT_MIN_STROKE_COLOR = config.get("BAR_SELECT_MIN_STROKE_COLOR")
    ...
    def select_bar_animation(self, bar: Bar):
        ...
        self._scene.add_sound("media/sfx/selection_sort/bar_select.mp3", gain=-10)
        self._scene.play(animation)
    ...
    def deselect_bar_animation(self, bar: Bar):
        ...
        animation = next(
            bar.get_deselect_animation(animation_speed=self._ANIMATION_SPEED_BAR_DESELECT)
        ).animation

        self._scene.play(animation)


    def set_min_bar_animation(self, bar: Bar):

        animation = next(
            bar.get_select_animation(
                fill_color=self._BAR_SELECT_MIN_FILL_COLOR,
                fill_opacity=self._BAR_SELECT_MIN_FILL_OPACITY,
                stroke_color=self._BAR_SELECT_MIN_STROKE_COLOR,
                stroke_opacity=self._BAR_SELECT_MIN_STROKE_OPACITY,
                stroke_width=self._BAR_SELECT_MIN_STROKE_WIDTH,
                keep_color=True,
                animation_speed=self._ANIMATION_SPEED_BAR_MIN_SELECT,
            )
        ).animation

        self._scene.add_sound("media/sfx/selection_sort/bar_min_select.mp3", gain=-18, time_offset=0.2)
        self._scene.play(animation)

    def refocus_min_bar_animation(self, min_bar: Bar, other_bar: Bar):

        animations = [
            next(min_bar.get_deselect_animation(animation_speed=self._ANIMATION_SPEED_BAR_DESELECT)).animation,
            next(
                other_bar.get_select_animation(
                    fill_color=self._BAR_SELECT_MIN_FILL_COLOR,
                    fill_opacity=self._BAR_SELECT_MIN_FILL_OPACITY,
                    stroke_color=self._BAR_SELECT_MIN_STROKE_COLOR,
                    stroke_opacity=self._BAR_SELECT_MIN_STROKE_OPACITY,
    ...
    def exchange_bars_animation(self, sorting_object: SortingObject, idx_left: int, idx_right: int):
        ...
        self._scene.add_sound("media/sfx/selection_sort/bar_exchange.mp3", gain=-13)
        self._scene.play(animation)
    ...
    def set_final_bar_animation(self, bar: Bar):
        ...
        self._scene.add_sound("media/sfx/selection_sort/bar_fixed.mp3", gain=-20)
        self._scene.play(animation)
